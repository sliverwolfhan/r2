#!/usr/bin/env bash
# 【蓝区】启动 simple_bt_runner (通过 ros2 launch at_r2_bt r2_bt_launch_blue.py)
# launch 内部已加载蓝区参数 weapon_grasp_params_blue.yaml 和 block_blue.yaml,
# 参数(grasp_count 等)自动生效。
# 用法:
#   ./run_bt_runner_blue.sh                    # 直接启动，默认行为树 r2_bt_blue.xml
#   ./run_bt_runner_blue.sh -b                 # 先 colcon build 整个工作空间再启动
#   ./run_bt_runner_blue.sh --build            # 同上
#   ./run_bt_runner_blue.sh -p "at_r2_bt"      # 仅编译指定包再启动
#   ./run_bt_runner_blue.sh grasp_head_2.xml   # 指定行为树 xml
#   ./run_bt_runner_blue.sh -b grasp_head_2.xml # 编译后用指定行为树启动
#   ./run_bt_runner_blue.sh -h                 # 帮助
#
# 透传参数到 ros2 launch:
#   ./run_bt_runner_blue.sh -- grasp_count:=2

set -e

ZONE="blue"
LAUNCH_FILE="r2_bt_launch_blue.py"
...
# 通过 launch 启动: r2_bt_launch_blue.py 内部已加载蓝区 weapon_grasp_params_blue.yaml
# 和 block_blue.yaml, 所以 grasp_count / grasp_start / block 坐标等参数自动生效。
# 行为树 xml 通过 launch 参数 bt_xml 传入 (默认 r2_bt_blue.xml)。
