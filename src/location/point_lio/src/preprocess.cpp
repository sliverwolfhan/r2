...
Preprocess::Preprocess() : lidar_type(AVIA), blind(0.01), point_filter_num(1), det_range(1000)
{
  ...
  // Body filter default parameters
  body_filter_en = false;
  body_min_x = -0.3;
  body_max_x = 0.3;
  body_min_y = -0.3;
  body_max_y = 0.3;
...
bool Preprocess::is_in_body_box(double x, double y, double z)
{
  if (!body_filter_en) return false;
  ...
  for (uint i = 1; i < plsize; i++) {
    if (
      ...
      if (valid_point_num % point_filter_num == 0) {
        ...
        // Body filter check
        if (is_in_body_box(pl_full[i].x, pl_full[i].y, pl_full[i].z)) continue;
  ...
  deque<double> & time_lidar, const int required_frame_num, int scan_count)
{
  ...
  if (lidar_type == VELO16) {
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      // Body filter check
      if (is_in_body_box(added_pt.x, added_pt.y, added_pt.z)) continue;
