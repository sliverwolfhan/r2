...
public:
    VirtualSerialPortNode() : Node("virtual_serial_port_node"), running_(true), current_mode_(0), mode_send_once_(false)
    {
        ...
        this->declare_parameter<double>("zone_x_max", 11.1);
        this->declare_parameter<double>("zone_y_min", 0.0);
        this->declare_parameter<double>("zone_y_max", 1.5);
        ...
        RCLCPP_INFO(this->get_logger(), "模式区域: x[%.1f, %.1f] y[%.1f, %.1f]",
            zone_x_min_, zone_x_max_, zone_y_min_, zone_y_max_);
    }
        ...
        while (running_) {
            try {
                ...
                if (std::chrono::duration_cast<std::chrono::seconds>(now - last_print_time).count() >= 1) {
                    RCLCPP_INFO(this->get_logger(), "位置 x: %.3f m, y: %.3f m yaw: %.3f", x, y, yaw);
                    last_print_time = now;
                }
