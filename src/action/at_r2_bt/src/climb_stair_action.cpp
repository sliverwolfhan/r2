...
BT::PortsList ClimbStairAction::providedPorts()
{
  return {
    ...
    // ===== 爬台阶期间朝向纠偏（只发 angular.z），默认关闭，不影响现有调用 =====
    BT::InputPort<bool>("yaw_correct_enable", false,
      "总开关：true 才在爬台阶期间发纠偏角速度；false/缺省完全不发"),
    BT::InputPort<double>("target_yaw",
      "纠偏目标朝向 (rad, map系)，一般绑爬台阶前的准备朝向 {prep_yaw}"),
...
double ClimbStairAction::normalizeAngle(double a)
{
  while (a > M_PI) {
    a -= 2.0 * M_PI;
  }
...
bool ClimbStairAction::ensureYawCorrection()
{
  ...
  if (!getInput("target_yaw", yaw_target_)) {
    RCLCPP_WARN(node_->get_logger(),
      "ClimbStair: yaw_correct_enable/offset_pub_enable=true 但缺少 [target_yaw]，本次不启用");
    yaw_correct_enable_ = false;
    offset_pub_enable_ = false;
    return false;
  }
  ...
  if (yaw_correct_enable_) {
    ...
    if (yaw_engage_threshold_ < yaw_deadband_) {
      RCLCPP_WARN(node_->get_logger(),
        "ClimbStair: yaw_engage_threshold(%.3f) < yaw_deadband(%.3f)，已夹到 deadband",
        yaw_engage_threshold_, yaw_deadband_);
      yaw_engage_threshold_ = yaw_deadband_;
    }
    yaw_engaged_ = false;  // 每次爬台阶重置迟滞状态
  ...
  if (!tf_buffer_) {
    ...
    if (!tf_buffer_) {
      ...
      RCLCPP_WARN(node_->get_logger(),
        "ClimbStair: 黑板无 tf_buffer，自建本地 TF listener");
    }
  }
  return true;
}
...
BT::NodeStatus ClimbStairAction::onStart()
{
  ...
  if (ensureYawCorrection()) {
    ...
    yaw_timer_ = node_->create_wall_timer(
      period_ns, std::bind(&ClimbStairAction::yawCorrectTimerCallback, this));
    RCLCPP_INFO(node_->get_logger(),
      "ClimbStair: 定时器启用 target_yaw=%.3f rate=%.1fHz | 纠偏=%s(kp=%.2f wz_max=%.2f topic=%s) | 偏移角=%s(topic=%s)",
      yaw_target_, rate_hz,
...
void ClimbStairAction::yawCorrectTimerCallback()
{
  {
    std::lock_guard<std::mutex> lock(yaw_timer_mutex_);
  ...
  try {
    ...
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
      "ClimbStair yaw-correct: TF lookup 失败 (%s)", e.what());
    return;
  }
...
void ClimbStairAction::stopYawTimer()
{
  {
    std::lock_guard<std::mutex> lock(yaw_timer_mutex_);
...
void ClimbStairAction::publishZeroCmd()
{
  if (cmd_vel_pub_) {
