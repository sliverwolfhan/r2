...
#ifndef NAV2_BT_PUBLISH_GOAL__PUBLISH_LIFT_CMD_ACTION_HPP_
#define NAV2_BT_PUBLISH_GOAL__PUBLISH_LIFT_CMD_ACTION_HPP_
...
/**
 * PublishLiftCmd: 向三区(架机区)发布一个整数指令, 控制升降/腿部动作。
 *
 *   command = 1  -> 抬升
 *   command = 2  -> 降到架机
 *   command = 3  -> 抬腿
 ...
 *   command = 5  -> 降下去
 *
 * 指令话题默认 /AT_R2/lift_cmd (std_msgs/Int32), 使用 transient_local(latched)
 * QoS, 确保下位机节点稍晚连接也能收到最新指令。
 *
 * 同步模式:
 ...
 * 输入端口:
 *   - node       (ROS node, 来自黑板)
 *   - command    (int)    要发布的指令: 1/2/3/4/5
 *   - topic      (string, 默认 "/AT_R2/lift_cmd")
 *   - wait_done  (bool,   默认 false)
 *   - done_topic (string, 默认 "/AT_R2/lift_done")
 *   - timeout    (double, 默认 0.0 = 无限等待) 等待反馈的超时秒数
 */
class PublishLiftCmdAction : public BT::StatefulActionNode
{
public:
  PublishLiftCmdAction(
...
#endif  // NAV2_BT_PUBLISH_GOAL__PUBLISH_LIFT_CMD_ACTION_HPP_
