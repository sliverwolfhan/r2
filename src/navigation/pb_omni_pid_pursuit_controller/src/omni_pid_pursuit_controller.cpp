  ...
  std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".translation_x_kp", rclcpp::ParameterValue(3.0));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".translation_x_ki", rclcpp::ParameterValue(0.1));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".translation_x_kd", rclcpp::ParameterValue(0.3));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".translation_y_kp", rclcpp::ParameterValue(3.0));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".translation_y_ki", rclcpp::ParameterValue(0.1));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".translation_y_kd", rclcpp::ParameterValue(0.3));
  declare_parameter_if_not_declared(
  ...
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".yaw_alignment_threshold", rclcpp::ParameterValue(0.15));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".yaw_full_suppress_angle", rclcpp::ParameterValue(0.6));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".velocity_smoothing_factor", rclcpp::ParameterValue(0.3));

  node->get_parameter(plugin_name_ + ".translation_x_kp", translation_x_kp_);
  node->get_parameter(plugin_name_ + ".translation_x_ki", translation_x_ki_);
  node->get_parameter(plugin_name_ + ".translation_x_kd", translation_x_kd_);
  node->get_parameter(plugin_name_ + ".translation_y_kp", translation_y_kp_);
  node->get_parameter(plugin_name_ + ".translation_y_ki", translation_y_ki_);
  node->get_parameter(plugin_name_ + ".translation_y_kd", translation_y_kd_);
  ...
  node->get_parameter(plugin_name_ + ".yaw_alignment_threshold", yaw_alignment_threshold_);
  node->get_parameter(plugin_name_ + ".yaw_full_suppress_angle", yaw_full_suppress_angle_);
  node->get_parameter(plugin_name_ + ".velocity_smoothing_factor", velocity_smoothing_factor_);
  ...
  move_pid_x_ = std::make_shared<PID>(
    control_duration_, v_linear_max_, v_linear_min_, translation_x_kp_, translation_x_kd_,
    translation_x_ki_);
  move_pid_y_ = std::make_shared<PID>(
    control_duration_, v_linear_max_, v_linear_min_, translation_y_kp_, translation_y_kd_,
    translation_y_ki_);
  ...
  nav2_core::GoalChecker * /*goal_checker*/)
{
  ...
  double error_x = carrot_pose.pose.position.x;
  double error_y = carrot_pose.pose.position.y;
  ...
  auto vel_x = move_pid_x_->calculate(error_x, 0);
  auto vel_y = move_pid_y_->calculate(error_y, 0);
  auto angular_vel = enable_rotation_ ? heading_pid_->calculate(angle_to_goal, 0) : 0.0;

  // Apply curvature and approach velocity scaling to the combined linear velocity
  double lin_vel = hypot(vel_x, vel_y);
  double original_lin_vel = lin_vel;

  applyCurvatureLimitation(transformed_plan, carrot_pose, lin_vel);

  applyApproachVelocityScaling(transformed_plan, lin_vel);

  double scale = (original_lin_vel > 1e-6) ? lin_vel / original_lin_vel : 0.0;
  vel_x *= scale;
  vel_y *= scale;

  // Suppress translation when yaw error is large (prioritize rotation alignment)
  double yaw_error = fabs(angle_to_goal);
  if (yaw_error > yaw_alignment_threshold_) {
    double yaw_scaling = std::max(
      0.0, 1.0 - (yaw_error - yaw_alignment_threshold_) / yaw_full_suppress_angle_);
    vel_x *= yaw_scaling;
    vel_y *= yaw_scaling;
  }
  ...
  if (!isCollisionDetected(costmap_frame_local_plan)) {
    // EMA low-pass filter: smoothed = alpha * new + (1 - alpha) * old
    double alpha = velocity_smoothing_factor_;
    cmd_vel.twist.linear.x = alpha * vel_x + (1.0 - alpha) * last_cmd_vx_;
    cmd_vel.twist.linear.y = alpha * vel_y + (1.0 - alpha) * last_cmd_vy_;
    cmd_vel.twist.angular.z = alpha * angular_vel + (1.0 - alpha) * last_cmd_wz_;
    last_cmd_vx_ = cmd_vel.twist.linear.x;
    last_cmd_vy_ = cmd_vel.twist.linear.y;
    last_cmd_wz_ = cmd_vel.twist.angular.z;
  } else {
    last_cmd_vx_ = 0.0;
    last_cmd_vy_ = 0.0;
    last_cmd_wz_ = 0.0;
  ...
  std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (const auto & parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      if (name == plugin_name_ + ".translation_x_kp") {
        translation_x_kp_ = parameter.as_double();
      } else if (name == plugin_name_ + ".translation_x_ki") {
        translation_x_ki_ = parameter.as_double();
      } else if (name == plugin_name_ + ".translation_x_kd") {
        translation_x_kd_ = parameter.as_double();
      } else if (name == plugin_name_ + ".translation_y_kp") {
        translation_y_kp_ = parameter.as_double();
      } else if (name == plugin_name_ + ".translation_y_ki") {
        translation_y_ki_ = parameter.as_double();
      } else if (name == plugin_name_ + ".translation_y_kd") {
        translation_y_kd_ = parameter.as_double();
      ...
      } else if (name == plugin_name_ + ".yaw_alignment_threshold") {
        yaw_alignment_threshold_ = parameter.as_double();
      } else if (name == plugin_name_ + ".yaw_full_suppress_angle") {
        yaw_full_suppress_angle_ = parameter.as_double();
      } else if (name == plugin_name_ + ".velocity_smoothing_factor") {
        velocity_smoothing_factor_ = parameter.as_double();
      }
