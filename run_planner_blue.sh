#!/usr/bin/env bash
# 【蓝区】启动 r2_meilin_planner (通过 ros2 launch r2_meilin_planner r2_planner_bringup_launch_blue.py)
# launch 内部 zone 默认 blue, 加载 planner_params.yaml, 发布 /r2_planner/plan。
# 用法:
#   ./run_planner_blue.sh                    # 直接启动 (zone=blue)
#   ./run_planner_blue.sh -b                 # 先 colcon build 整个工作空间再启动
#   ./run_planner_blue.sh --build            # 同上
#   ./run_planner_blue.sh -p "r2_meilin_planner"  # 仅编译指定包再启动
#   ./run_planner_blue.sh -h                 # 帮助
#
# 透传参数到 ros2 launch (覆盖 ignore_height / can_climb_400 / params_file 等):
#   ./run_planner_blue.sh -- can_climb_400:=false

set -e

LAUNCH_FILE="r2_planner_bringup_launch_blue.py"

usage() {
  cat <<EOF
Usage: $(basename "$0") [-b|--build] [-p|--packages "pkg1 pkg2"] [-h|--help] [-- <ros2 launch args>]

  【蓝区】固定使用 ${LAUNCH_FILE} (zone=blue)
