#!/usr/bin/env bash
# 启动 launch_pack arm_real
# 用法:
#   ./run_arm_real.sh           # 直接启动，不编译
#   ./run_arm_real.sh -b        # 先 colcon build 再启动
#   ./run_arm_real.sh --build   # 同上
#   ./run_arm_real.sh -h        # 帮助
#
# 透传参数到 ros2 launch:
#   ./run_arm_real.sh -- use_sim_time:=true
#   ./run_arm_real.sh -b -- use_sim_time:=true
...
Usage: $(basename "$0") [-b|--build] [-p|--packages "pkg1 pkg2"] [-h|--help] [-- <ros2 launch args>]
  ...
  -p, --packages    仅 build 指定的包，例如: -p "launch_pack arm_task"
  -h, --help        显示帮助
...
if ! verify_build_artifacts "launch_pack"; then
  exit 1
fi

echo "==> ros2 launch launch_pack arm_real.launch.py ${LAUNCH_ARGS[*]}"
exec ros2 launch launch_pack arm_real.launch.py "${LAUNCH_ARGS[@]}"
