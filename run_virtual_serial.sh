#!/usr/bin/env bash
# 启动 virtual_serial_port_node
# 用法:
#   ./run_virtual_serial.sh           # 直接启动，不编译
#   ./run_virtual_serial.sh -b        # 先 colcon build 整个工作空间再启动
#   ./run_virtual_serial.sh --build   # 同上
#   ./run_virtual_serial.sh -p "virtual_serial_port"  # 仅编译指定包再启动
#   ./run_virtual_serial.sh -h        # 帮助
#
# 透传参数到 ros2 run:
#   ./run_virtual_serial.sh -- --ros-args -p baudrate:=115200
#   ./run_virtual_serial.sh -b -- --ros-args -p baudrate:=115200
...
Usage: $(basename "$0") [-b|--build] [-p|--packages "pkg1 pkg2"] [-h|--help] [-- <ros2 run args>]
  ...
  -p, --packages    仅 build 指定的包，例如: -p "virtual_serial_port"
  -h, --help        显示帮助
  --                后续参数原样传给 ros2 run
EOF
}
...
RUN_PKG="virtual_serial_port"
RUN_EXE="virtual_serial_port_node_test"
...
if ! verify_build_artifacts "virtual_serial_port"; then
  exit 1
fi
