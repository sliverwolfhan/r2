#!/usr/bin/env bash
# 【蓝区】启动 at_r2_nav_bringup 导航 (at_navigation_launch_blue.py)
# launch 内部默认 world=rc_2026_blue, params=nav2_params_mppi_blue.yaml。
# 用法:
#   ./run_navigation_blue.sh           # 直接启动，不编译
#   ./run_navigation_blue.sh -b        # 先 colcon build 再启动
#   ./run_navigation_blue.sh --build   # 同上
#   ./run_navigation_blue.sh -h        # 帮助
#
# 透传参数到 ros2 launch:
#   ./run_navigation_blue.sh -- use_sim_time:=true
#   ./run_navigation_blue.sh -b -- use_sim_time:=true

set -e

LAUNCH_FILE="at_navigation_launch_blue.py"
...
echo "==> [蓝区] ros2 launch at_r2_nav_bringup ${LAUNCH_FILE} ${LAUNCH_ARGS[*]}"
